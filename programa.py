...
def suma_1_al_10():
    ...
    print ("la suma de los numero del 1 al 10 es:", suma)# Se devuelve el resultado de la suma
if __name__ == "__main__":
...
def imprimir_datos_personales(nombre, edad, estatura):
    print(nombre)
    print(edad)
    print(estatura) # Se imprimen en pantalla los datos personales recibidos como argumentos
if __name__ == "__main__":
    # Se definen las variables con los datos personales
    nombre = "Bryam"
    edad = 30
    estatura = 1.75
    imprimir_datos_personales("Bryam", 30 , 1.75)    # Se llama a la función imprimir_datos_personales() para mostrar los datos

# 4 Escribe un programa que determine si un número ingresado por el usuario es par o impar.
def par_o_impar(numero):
    if num %2 == 0:# Se verifica si el número es divisible por 2
       print (num, "Es par") # Si es divisible, se devuelve "par"
    else:
       print (num, "Es impar")  # Si no es divisible, se devuelve "impar"
...
if __name__ == "__main__":
    ...
    print(f"El area del ciculo es: {radio} es {area:.2f}.") # Se imprime el área calculada del círculo

# 6 Define una función que reciba dos números como argumentos y devuelva su suma.
def suma(a, b):
    return num1 + num2 # Se devuelve la suma de los dos números recibidos como argumentos
if __name__ == "__main__":
